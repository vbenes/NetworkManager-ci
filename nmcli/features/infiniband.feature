Feature: nmcli: inf
    ...
    * Add connection type "infiniband" named "inf" for device "mlx5_ib0"
    * Bring "up" connection "inf"
    Then "inet 172" is visible with command "ip a s mlx5_ib0"
     ...
     * Submit "mlx5_ib0" in editor
     * Expect "Do you want to provide them\? \(yes\/no\) \[yes\]"
     * Submit "no" in editor
     * Dismiss IP configuration in editor
     * Dismiss Proxy configuration in editor
     * Bring "up" connection "infiniband-mlx5_ib0"
    Then "inet 172" is visible with command "ip a s mlx5_ib0"
    ...
    * Bring "down" connection "inf"
    Then "inet 172" is not visible with command "ip a s mlx5_ib0"
    ...
    * Add connection type "infiniband" named "inf" for device "mlx5_ib0"
    * Add infiniband port named "inf.8002" for device "mlx5_ib0.8002" with parent "mlx5_ib0" and p-key "0x8002"
    * Bring "up" connection "inf"
    * Bring "up" connection "inf.8002"
    Then "inet 172" is visible with command "ip a s mlx5_ib0.8002"
     ...
     * Dismiss Proxy configuration in editor
     * Bring "up" connection "inf"
     * Bring "up" connection "infiniband-mlx5_ib0.8002"
    Then "inet 172" is visible with command "ip a s mlx5_ib0.8002"
    ...
    * Bring "down" connection "inf.8002"
    Then "inet 172" is visible with command "ip a s mlx5_ib0"
    Then "inet 172" is not visible with command "ip a s mlx5_ib0.8002"
    ...
    * Reboot
     ...
     * Bring "up" connection "bond0"
     Then "inet 172" is visible with command "ip a s nm-bond"
      ...
      And "mlx5_ib0:infiniband:connected:inf" is visible with command "nmcli -t -f DEVICE,TYPE,STATE,CONNECTION device" in "5" seconds
      And Check bond "nm-bond" link state is "up"
      And Check slave "mlx5_ib0" in bond "nm-bond" in proc
    ...
    * Add a new connection of type "infiniband" and options "con-name inf ifname mlx5_ib0 infiniband.transport-mode datagram"
    * Add a new connection of type "infiniband" and options "con-name inf2 ifname mlx5_ib0 infiniband.transport-mode connected"
    * Bring "up" connection "inf"
    ...
    * Execute "pkill journalctl; sleep 2"
    When "taking down device.*mlx5_ib0/mode' to 'connected'" is visible with command "egrep 'taking|datagram' /tmp/journal.txt"
    * Run child "journalctl -f > /tmp/journal.txt"
    * Bring "up" connection "inf"
    * Execute "sleep 2;pkill journalctl"
    Then "taking down device.*mlx5_ib0/mode' to 'datagram'" is visible with command "egrep 'taking|datagram' /tmp/journal.txt"
